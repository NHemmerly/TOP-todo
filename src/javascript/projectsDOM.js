...
export function createProjectCard(project) {
    const newCard = domElement('a', {class: 'block min-w-sm p-6 bg-white border border-yellow-200 rounded-lg shadow hover:bg-yellow-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700'},
                        domElement('h3', {class: 'mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white'}, `${project.title}`),
                        domElement('h5', {class: 'font-normal text-gray-700 dark:text-gray-400'}, `${project.dueDate}`))
